...
class Hysteria2(BaseClient):
    def __init__(self, clients_dir: str, file: str):
        super().__init__(clients_dir, {"win": "hysteria", "unix": "hysteria"}, file)
        self._cmd: dict = {
            "win_debug": [
                f"{self._clients_dir}hysteria/hysteria2.exe",
                "-c",
                self._config_file,
            ],
            "win": [
            ...
            "unix_debug": [
                f"{self._clients_dir}hysteria/hysteria2",
                "-c",
                self._config_file,
            ],
            "unix": [f"{self._clients_dir}hysteria/hysteria2", "-c", self._config_file],
        }
